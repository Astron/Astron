...
STATESERVER_OBJECT_GENERATE_WITH_REQUIRED(2001):
	(uint32 parent_id, uint32 zone_id, uint16 dclass_id, uint32 do_id, ...)
...
STATESERVER_OBJECT_UPDATE_FIELD(2004):
	(uint32 do_id, uint16 field, FIELD)
...
STATESERVER_OBJECT_UPDATE_FIELD_MULTIPLE(2005):
	(uint32 do_id, uint16 num_fields, [uint16 field, FIELD]*num_fields)
As above, but this contains multiple field updates in one message, with the
intention that the updates will be processed atomically.

STATESERVER_OBJECT_DELETE_RAM(2007):
	(uint32 do_id)
Remove the object from the State Server. This message does not affect a stored
copy on the database, if one exists. The object will duly broadcast its
deletion to any AIs, owners, or zones.
...
STATESERVER_OBJECT_CHANGE_ZONE(2009):
	(uint32 do_id, uint32 new_parent, uint32 new_zone, uint32 old_parent,
	 uint32 old_zone)
This message is SENT BY THE OBJECT when processing a SET_ZONE. It tells
everything that can see the object where the object is going.
STATESERVER_OBJECT_ENTERZONE_WITH_REQUIRED_OTHER(2066):
	(uint16 dclass_id, uint32 do_id, uint32 parent_id, uint32 zone_id, ...)
This message is SENT BY THE OBJECT when processing a SET_ZONE. It tells
the new zone about the object's entry. The message is ordered slightly
differently, but is otherwise identical to the behavior of
...
STATESERVER_OBJECT_LOCATE(2022):
	(uint32 context)
This message may be used to ask an object for its location. The reply is:
...
STATESERVER_OBJECT_QUERY_FIELD(2024):
	(uint32 do_id, uint16 field_id, uint32 context)
This message may be used to ask an object for the value of a field. Reply:
STATESERVER_OBJECT_QUERY_FIELD_RESP(2062):
	(uint32 do_id, uint16 field_id, uint32 context, uint8 success, [FIELD])
...
STATESERVER_OBJECT_ENTER_AI_RECV(2067):
	(uint32 parent_id, uint32 zone_id, uint16 class_id, uint32 do_id, ...)
The new AI Server is told of the object's arrival.
The ... is as in REQUIRED_OTHER.
STATESERVER_OBJECT_LEAVING_AI_INTEREST(2033):
	(uint32 do_id)
Sent to the old AI Server to notify it of the object's departure or deletion.

STATESERVER_OBJECT_DELETE_DISK(2060): Handled by DBSS, not SS.

STATESERVER_OBJECT_SET_OWNER_RECV(2069):
	(uint64 owner_channel)
Set the channel of the object owner. This is the channel of the Client Agent
connection object where ownrecv messages will be forwarded. Similar to
changing zone, this will generate some traffic:
STATESERVER_OBJECT_CHANGE_OWNER_RECV(2069):
	(uint32 do_id, uint64 new_owner_channel, uint64 old_owner_channel)
Sent to the old owner.
STATESERVER_OBJECT_ENTER_OWNER_RECV(2068):
	(uint32 parent_id, uint32 zone_id, uint16 dclass_id, uint32 do_id, ...)
The new owner is told of the object's arrival. The ... is as in REQUIRED_OTHER.

STATESERVER_OBJECT_QUERY_FIELDS(2080):
	(uint32 do_id, uint32 context, uint16 field_ids[...])
Ask the object for all of its fields.
N.B. there is no length tag on field_ids; the end of the message signifies the
end of the field ID list.
STATESERVER_OBJECT_QUERY_FIELDS_RESP(2081):
	(uint32 do_id, uint32 context, uint8 success, [uint16 field_id, FIELD])
The reply sent back to the requester. Again, there is no length tag.

STATESERVER_OBJECT_QUERY_ALL(2020):
	(uint32 context)
This message queries all information from the object.
Response:
STATESERVER_OBJECT_QUERY_ALL_RESP(2030):
	(uint32 context, uint32 parent_id, uint32 zone_id, uint16 dclass_id,
	 uint32 do_id, ...)
The ... is like that of REQUIRED_OTHER, above.

STATESERVER_OBJECT_QUERY_MANAGING_AI(2083):
	()
Internally used: The object has just changed parents. It wants to know if its
...
STATESERVER_OBJECT_NOTIFY_MANAGING_AI(2047):
	(uint32 parent_id, uint64 ai_channel)
Internally used: Broadcast by a parent_id to all of its children (the channel
