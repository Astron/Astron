...
HTTPServer::HTTPServer(string str_bind, string web_path)
{    
    s_webPath = web_path;
    
    if(s_webPath == "FROZEN") initFrozen();
    
    std::vector<std::string> bindParts;
    boost::split(bindParts, str_bind, boost::is_any_of(":"));
    
    tcp::resolver resolver(io_service);
    tcp::resolver::query query(bindParts[0], // address
                                bindParts.size() > 1 ? bindParts[1] : "80" // port defaults to 80
                                );
