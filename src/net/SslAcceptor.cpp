...
void SslAcceptor::handle_accept(ssl::stream<tcp::socket> *socket, const boost::system::error_code &ec)
{
	if(!m_started)
	{
...
void SslAcceptor::handle_handshake(ssl::stream<tcp::socket> *socket, const boost::system::error_code &ec)
{
	if(ec.value() != 0)
	{
